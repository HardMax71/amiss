...
- `invalid-reference`: the destination cannot name a target: it escapes the repository, carries a backslash, an encoded separator, or control bytes, or is a URL that is not well formed; fix the destination
- `target-declared-untracked`: a tracked ignore file says the repository does not keep the path this reference names, either by naming that path or by emptying the directory the file sits in, so no tree can answer for the link; the reference is recorded and counted, never cleared
...
- `unsupported-document-format`: a document this run discovered was not scanned: no parser for its markup, a policy include with no grammar bound, bytes that do not decode as the format requires, markup its grammar refuses, or a per-document ceiling crossed; the document row's reason says which, and the file's references are never checked
- `unsupported-target-kind`: the reference resolves to a symlink or submodule, or to a path beneath one, which Amiss does not follow; the boundary is declared instead of crossed
