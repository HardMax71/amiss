...
| --- | --- | --- |
| Command | `amiss check` compares a base commit with either a candidate commit or the staged index, and `amiss --version` reports the binary's version and engine digest. The command grammar is closed at those two forms. | [CLI parser](https://github.com/HardMax71/amiss/blob/main/crates/amiss/src/invocation.rs) |
