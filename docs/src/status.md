...
| Cross-repository relations | The provider-neutral controller can atomically validate and freeze at most 1,024 operator-owned two-subject relations. Each subject binds an exact provider/integration/repository scope, target ref, object format, credential reference, closed projection selector, and independent limits; the relation binds aggregate limits and exact status destinations. Either authenticated subject selects every owned relation in stable identity order. Acquisition, immutable revision freezing, evaluation evidence, persistence, service configuration, and status publication are not built yet. | [Cross-repository relations](cross-repository-relations.md) |
