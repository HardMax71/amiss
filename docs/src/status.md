...
| --- | --- | --- |
| Command | `amiss check` compares a base commit with either a candidate commit or the staged index, `amiss fix` applies the staged evaluation's proof-gated fixes to the working tree, `amiss claim` authors a value claim proven against the working tree before it is printed, `amiss adopt` mints an adoption-debt snapshot from a commit pair's eligible findings, and `amiss --version` reports the binary's version, with its engine digest when the binary can read itself. The command grammar is closed at those five forms. | [CLI parser](https://github.com/HardMax71/amiss/blob/main/crates/amiss/src/invocation.rs) |
