...
| --- | --- | --- |
| Command | `amiss check` compares a base commit with either a candidate commit or the staged index, `amiss fix` applies the staged evaluation's proof-gated fixes to the working tree, `amiss claim` authors a value claim proven against the working tree before it is printed, `amiss adopt` mints an adoption-debt snapshot from a commit pair's eligible findings, `amiss external-plan` derives the delegated-evidence destination delta from a written report without opening a repository, `amiss external-assess` judges that plan against a producer's observations under a fixed offline policy, `amiss render` projects one validated report without evaluating it again, `amiss refs` returns the exact candidate occurrences that refer to one repository path, and `amiss --version` reports the binary's version, with its engine digest when the binary can read itself. The command grammar is closed at those nine forms. | [CLI parser](https://github.com/HardMax71/amiss/blob/main/crates/amiss/src/invocation.rs) |
