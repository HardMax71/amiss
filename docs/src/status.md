...
| Documents | Built-in discovery covers Markdown, GFM, MDX, AsciiDoc, reStructuredText, six extensionless Markdown basenames, and two plain-advisory basenames. Repository policy may add exact paths or trees narrowed by one exact suffix, each optionally bound to one built-in grammar, without installing another parser. | [Classifier](https://github.com/HardMax71/amiss/blob/main/crates/amiss-scan/src/document.rs) |
