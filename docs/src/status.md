...
| Locale coverage audits | One closed digest-bound plan binds an accepted report and exact docs candidate to a site, source/target locale pair, channel, optional version, independently selected page-inventory producer, and operator-owned coverage and fallback policy. A matching evidence contract carries independently complete source and target inventories; every target page is target-owned or declares an exact source fallback class and digest. The pure offline assessment reports proved missing/orphan pages and allowed, unauthorized, source-mismatched, or source-unproven fallback rows. Only an exhaustive clean comparison matches. Page keys remain opaque, and no contract makes a translation or freshness claim. Command and controller intake are not built yet. | [Locale coverage audits](locale-coverage.md) |
