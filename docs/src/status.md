...
| Publication audit wire | Closed digest-bound contracts model an operator-owned publication plan, one provider-normalized successful-deployment receipt, and a conservative offline matched/refuted/unproven assessment. They bind the accepted report and exact docs, target, completed-site, product, deployment, workflow, producer, and evaluator identities. No command or controller lane acquires, stores, or publishes them yet. | [Publication audits](publication.md) |
