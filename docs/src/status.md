...
| Semantic evidence | A separate 16 MiB envelope binds one external producer and at most 100,000 canonical observations to the exact candidate and optional source report. Unknown observation kinds are inert. The sealed controls request accepts the envelope, and the first compiled consumer resolves unique prefixless Sphinx labels from one complete candidate-bound inventory set; the public command supplies none. Provider plans can produce that set from bounded operator-local or cached Sphinx inventories. | [Contract](semantic-evidence.md), [producer](https://github.com/HardMax71/amiss/blob/main/controller/src/intersphinx.rs), and [consumer](https://github.com/HardMax71/amiss/blob/main/crates/amiss-scan/src/semantic.rs) |
...
opaque; leading-slash site routes, code symbols, live URLs, and references to other
repositories are not validated under those systems' semantics. Their visible
