...
| --- | --- | --- |
| Command | `amiss check` compares a base commit with either a candidate commit or the staged index, `amiss fix` applies the staged evaluation's proof-gated fixes to the working tree, `amiss claim` authors a value claim proven against the working tree before it is printed, `amiss policy-include` authors one validated exact-suffix policy row and can preview its staged matches, `amiss adopt` mints an adoption-debt snapshot from a commit pair's eligible findings, `amiss external-plan` derives the delegated-evidence destination delta from a written report without opening a repository, `amiss external-assess` judges that plan against a producer's observations under a fixed offline policy, `amiss render` projects one validated report without evaluating it again, `amiss refs` returns the exact candidate occurrences that refer to one repository path, `amiss --help` prints that closed grammar, and `amiss --version` reports the binary's version, with its engine digest when the binary can read itself. The command grammar is closed at those eleven forms. | [CLI parser](https://github.com/HardMax71/amiss/blob/main/crates/amiss/src/invocation.rs) |
...
| Semantic evidence | A separate 16 MiB envelope binds one external producer and at most 100,000 canonical observations to the exact candidate and optional source report. Every sealed value also carries the controller-plan-owned context digest its producer must reproduce. Unknown observation kinds are inert. Compiled consumers resolve unique prefixless Sphinx labels and exact repository-backed or source-attributed generated candidate site routes, anchors, fragment-aware terminal redirects, and evidence-gated rendered navigation reachability; conflicting routes and broken redirects retain their routing source as typed findings. The public command supplies no evidence. Provider plans produce the bounded operator-local or cached Sphinx inventory set. Trusted acquisition can add strictly checked candidate-specific pre-scan envelopes. The controller can derive routes, anchors, and navigation from a pinned mdBook renderer context and caller-opened completed HTML output, bound to an exact configuration path, publication prefix, locale, and version, while the built-in acquisitions remain behavior-neutral. | [Contract](semantic-evidence.md), [inventory producer](https://github.com/HardMax71/amiss/blob/main/controller/src/intersphinx.rs), [mdBook producer](https://github.com/HardMax71/amiss/blob/main/controller/src/mdbook.rs), and [consumer](https://github.com/HardMax71/amiss/blob/main/crates/amiss-scan/src/semantic.rs) |
...
candidate-ref fields remain caller assertions, and it supplies no semantic evidence. The
