...
| Locale coverage audits | One closed digest-bound plan binds an accepted report and exact docs candidate to a site, locale pair, independently selected producer, and operator-owned coverage, fallback, and optional target-lineage policy. Evidence carries independently complete source and target inventories; every target page is target-owned with nullable exact source lineage or declares an exact fallback class and source digest. The offline assessment reports proved missing/orphan pages, fallback status, and current/stale/unproven target lineage. Only an exhaustive clean required comparison matches. Exact lineage proves digest equality, never translation quality or semantic equivalence. Command and controller intake are not built yet. | [Locale coverage audits](locale-coverage.md) |
