...
| Semantic evidence | A separate 16 MiB envelope binds one external producer and at most 100,000 canonical observations to the exact candidate and optional source report. Unknown observation kinds are inert. Compiled consumers resolve unique prefixless Sphinx labels and exact source-bound candidate site routes, anchors, and fragment-aware terminal redirects; the public command supplies no evidence. Provider plans produce the bounded operator-local or cached Sphinx inventory set. Trusted acquisition can add strictly checked candidate-specific pre-scan envelopes. The controller can produce one from a pinned mdBook renderer context and caller-opened completed HTML output, while the built-in acquisitions remain behavior-neutral. | [Contract](semantic-evidence.md), [inventory producer](https://github.com/HardMax71/amiss/blob/main/controller/src/intersphinx.rs), [mdBook producer](https://github.com/HardMax71/amiss/blob/main/controller/src/mdbook.rs), and [consumer](https://github.com/HardMax71/amiss/blob/main/crates/amiss-scan/src/semantic.rs) |
