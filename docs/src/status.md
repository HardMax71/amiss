...
| Locale coverage audits | One closed digest-bound plan binds an accepted report and exact docs candidate to a site, source/target locale pair, channel, optional version, independently selected page-inventory producer, and operator-owned all-source or named-required policy. A matching evidence contract repeats those facts and carries separately complete source and target key-to-resource-digest inventories under one combined 100,000-row/16 MiB budget. The pure offline assessment reports proved missing/orphan page keys and marks lower-bound results incomplete; only an exhaustive clean comparison matches. Page keys remain opaque, and no contract makes a translation, fallback, or freshness claim. Command and controller intake are not built yet. | [Locale coverage audits](locale-coverage.md) |
