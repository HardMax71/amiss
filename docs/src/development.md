...
with everything nested in it and a parent and its own child are never compared. The workspace has
none since [#1133](https://github.com/HardMax71/amiss/issues/1133) was worked through, so the job
denies the lint and a new cycle fails it.
...
`THIRD_PARTY_LICENSES.txt`, their `SHA256SUMS`, and the sigstore bundle attesting that file attach
to the draft, so a release that fails the matrix never publishes a binary.
