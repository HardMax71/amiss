...
and the observed lower bound. A JSON input that fails its strict reader, such as the repository policy, also
carries `json_path`, the member the defect sits at, as in `$.document_includes[0].kind`, and
the human error line prints it after the file. [The report](report.md) has the exact shape. The human output
