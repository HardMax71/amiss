...
Each section is the exit evidence for one closed phase: what the phase claimed, and what
was recorded to justify calling it done. The page is append-only, so an exit decision
stays inspectable long after the work stops being interesting. Current work is in the
[Roadmap](roadmap.md), the factual boundary of the product is in
