...
keys sorted, one byte sequence per possible document, so the same input produces the same
bytes on Linux, macOS, and Windows. Duplicate keys are rejected everywhere on input, and
...
cut short: if the number of findings crosses the contract ceiling, the run ends incomplete
with `OUTPUT_LIMIT_EXCEEDED` instead of quietly shortening the list.
