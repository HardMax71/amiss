...
Related introduced problems become one `fix` per target, and so do problems whose
attribution is `unknown`, since `enforce-introduced` keeps those blocking. Changed targets
under unchanged prose become one `check`, and pre-existing failures and warnings become one `existing` per
...
byte never reads as the character sharing its value, and states any overflow explicitly. It keeps
raw totals and prints each sentence once as a `note` line, for every error code and every finding
kind the run reports, while the rows themselves stay in JSON. The places
...
base for the same reason, the row's own words and then its
