    ...
    for (bytes, encoded) in [
        ...
        let artifact = InputArtifact {
            ...
            inputs: vec![InputArtifactRow::<amiss_wire::model::ArtifactId> {
                ...
                template_bytes: Cow::Borrowed(bytes),
                envelope_bytes: Cow::Borrowed(bytes),
        ...
        for field in ["template_bytes_base64", "envelope_bytes_base64"] {
            ...
            for invalid in [
                ...
                assert!(
                    amiss_wire::read_json::<InputArtifact<'static>>(tampered.as_bytes(), u64::MAX)
                        .is_err(),
                    "{field}: {invalid}"
                );
            }
            if encoded.ends_with('=') {
                ...
                assert!(
                    amiss_wire::read_json::<InputArtifact<'static>>(unpadded.as_bytes(), u64::MAX)
                        .is_err()
                );
            }
        }
        let exact = u64::try_from(wire.len()).unwrap();
        let parsed: InputArtifact<'static> = amiss_wire::read_json(wire.as_bytes(), exact).unwrap();
        assert!(matches!(parsed.inputs[0].template_bytes, Cow::Owned(_)));
        assert!(matches!(parsed.inputs[0].envelope_bytes, Cow::Owned(_)));
        ...
        assert!(
            amiss_wire::read_json::<InputArtifact<'static>>(wire.as_bytes(), exact - 1).is_err()
        );
        for invalid in [
            ...
            assert!(
                amiss_wire::read_json::<InputArtifact<'static>>(invalid.as_bytes(), u64::MAX)
                    .is_err()
            );
        }
    }
