...
use self::journal::{
    ENTRY_LENGTH_BYTES, ENTRY_SCHEMA, MAX_ACTIONS_PER_BINDING, MAX_ENTRY_BYTES, MAX_ROOT_BYTES,
    ROOT_SCHEMA, decode_entry, decode_metadata, empty_metadata, encode_entry, encode_metadata,
    journal_byte_limit,
};
    ...
    fn load_metadata(&self) -> Result<RootMetadata, RelationScheduleStoreError> {
        ...
        let bytes =
            amiss_controller_files::read_bounded(&self.root.join(METADATA_FILE), MAX_ROOT_BYTES)
                .map_err(RelationScheduleStoreError::Io)?;
        decode_metadata(&bytes, self.max_bindings)
    }
