...
fn every_workflow_run_binding_clause_fails_closed() {
    ...
    let defects: [fn(&mut WorkflowRunPage); 14] = [
        ...
        |page| page.workflow_runs[0].status = "queued".parse().unwrap(),
        |page| page.workflow_runs[0].conclusion = Some("failure".parse().unwrap()),
...
fn run_page(candidate: &Oid) -> WorkflowRunPage {
    WorkflowRunPage {
        ...
        workflow_runs: vec![WorkflowRunRecord {
            ...
            status: "completed".parse().unwrap(),
            conclusion: Some("success".parse().unwrap()),
            workflow_id: 321,
            run_attempt: 2,
