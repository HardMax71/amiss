...
- say which references a run declined and why
- read a destination that opens a template expression as one
- read a site route against the base a repository declares
...
- *(action)* drop the case expansion macOS bash cannot read
...
- fold the scanner fuzz crate into the workspace
- [**breaking**] mint wire compatibility 2 with one occurrence per row
- spell a sealed payload once
- give validated newtypes a const literal constructor
- read validated newtypes through serde try_from
- *(wire)* read the report and the last plain documents through the traits
- *(wire)* seal the open families with the envelope reader
- *(wire)* read every plain document through one Document trait
- *(locale)* show the two layouts a context can describe
- *(cli)* stage locale coverage evidence with amiss locale-inventory
- build fixture digests with the shared law
- *(wire)* seal every digest-bound document through one envelope
- *(locale)* judge coverage plans from the command line
- preserve domain types across configuration, storage, and providers
- replace custom Serde implementations with derives
- replace JSON and digest wrappers with native libraries
- *(wire)* keep public report ingress fully typed
- Skip unrelated CI work and shrink executable test fixtures
- *(cli)* retain typed reference occurrences
- *(wire)* share tagged resolution models
- *(wire)* close report row models
- *(wire)* separate semantic construction from output
- *(wire)* keep semantic observation ownership explicit
- *(scan)* consume typed controls without envelope round-trips
- *(wire)* close semantic template and envelope payloads
- *(wire)* close semantic producer kinds across the data flow
- *(wire)* carry concrete policy controls through sealed requests
- *(spec)* reject string-tag shape drift across public readers
- *(wire)* remove the handwritten canonical JSON writer
- Use library JSON writers in bootstrap and CLI fixtures
- Retain generated semantic envelopes beside their canonical bytes
- Send bounded external CLI inputs directly to artifact validators
- Read reference projections with Serde and preserve input extensions
- Pass bounded report bytes directly to CLI validators
- Read report envelopes directly through Serde
...
- strip release symbols and skip the read on a scan memo hit
- *(scan)* read a locale page inventory out of the candidate tree
- give the domain-separated digest one home
- *(scan)* name the control admission law once
- compile the test suites once instead of many times
- reuse production wire models and control fixtures
- *(scan)* carry closed exception applications into reports
- Share closed finding evidence across report and adoption flows
- Use closed Serde tags for projection and exception details
- *(wire)* close finding key scopes
- *(wire)* borrow closed report identity
- *(wire)* type supplied semantic request envelopes
- *(scan)* consume closed semantic observations
- *(wire)* keep supplied trusted time typed end to end
- Use library JSON writers in scanner and Markdown fixtures
- Consume generated semantic envelopes directly in the scanner
- Hash site identities from existing evidence kinds with Serde
- Hash blob evidence from typed Serde inputs
...
- parse the Markdown profile with pulldown-cmark
- *(locale)* retain a validated coverage audit in the controller
- eliminate transient clones in domain flows
- infer Serde bounds and simplify optional fields
- Enforce report count bounds through Serde adapters
- *(bootstrap)* validate complete typed reports
- *(wire)* share projection source models
- *(wire)* close report engine and summary metadata
- *(report)* share typed result validation
- *(wire)* close report control provenance
- *(wire)* require strings for observation and resolution tags
- *(wire)* require strings for finding and evidence tags
- *(wire)* require strings for report context tags
- *(wire)* require strings for sidecar assessment tags
- *(wire)* require strings for external and semantic tags
- *(wire)* require strings for control and manifest tags
- *(wire)* require strings for request and Git tags
- *(wire)* serialize protocol fixtures with the canonical library
- Use library JSON writers in wire contract fixtures
- Derive serialization for strict JSON values
- Stream semantic payload digests through the canonical writer
- Bind relation sources directly through their Serde model
- Let Serde enforce policy assertion presence
- Remove the unused manual field decoding API
- Return external evidence with its complete-input digest
- Decode external envelopes with shared Serde input checks
- Validate decoded semantic evidence at bootstrap capture
- Read bootstrap reports through typed acceptance stages
- Share report identity projections between readers
- Read sealed report controls through existing Serde models
- Use string-only library derives for control tags
- Reject unbound report envelope fields with Serde
- Remove redundant semantic fixture digest assertion
- Keep semantic observations object-shaped under Serde
- Write typed semantic observations without JSON value adapters
...
- Validate candidate snapshot shapes in index-mode reports
- Derive sealed report identities without rebuilding JSON
- Decode captured bootstrap controls directly with Serde
