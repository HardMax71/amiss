...
impl EditMap {
    ...
    pub fn consume(&mut self, events: &mut Vec<Event>) {
        ...
        let mut source = mem::take(events).into_iter();
        let mut source_index = 0;
        let mut rebuilt = Vec::with_capacity(source.len() + add_acc - remove_acc);
        for (at, remove, add) in &mut self.map {
            rebuilt.extend(source.by_ref().take(*at - source_index));
            source_index = *at;
            source.by_ref().take(*remove).for_each(drop);
            source_index += *remove;
            rebuilt.append(add);
        }
