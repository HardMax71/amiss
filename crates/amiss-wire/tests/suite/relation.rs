...
use amiss_wire::relation::{
    PLAN_PAYLOAD_SCHEMA, RELATION_DOCUMENT_BYTES, RelationIdentity, RelationPlan, RelationSnapshot,
    RelationSubject, parse_plan, plan,
};
...
#[test]
fn relation_plan_refuses_tampering_open_shapes_and_oversized_documents() {
    let value = plan(&relation_plan()).unwrap();
    let bytes = json::canonical(&value);
    let parsed = parse_plan(&bytes).unwrap();
