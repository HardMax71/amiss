...
use amiss_wire::publication::{
    ...
    PublicationOutcome, PublicationPlan, PublicationResource,
};
use js_int::MAX_SAFE_INT;

pub(super) fn publication_evidence() -> PublicationEvidence {
    let planned = publication_plan();
    let planned_value = planned.emit().unwrap();
    let planned = PublicationPlan::parse(&planned_value).unwrap();
    PublicationEvidence {
...
fn publication_evidence_round_trips_with_its_plan_and_payload_digests() {
    ...
    let value = expected.emit().unwrap();
    let bytes = value;
    let parsed = PublicationEvidence::parse(&bytes).unwrap();
    ...
    let example = PublicationEvidence::parse(&example_bytes).unwrap();
    let planned =
...
fn publication_evidence_refuses_non_success_and_unsafe_attempts() {
    ...
    for provider_run_attempt in [0, maximum + 1, u64::MAX] {
        ...
        let error = invalid.emit().unwrap_err();
        assert_eq!(error.path, "$.payload.deployment.provider_run_attempt");
        assert_eq!(error.kind, ErrorKind::InvalidValue);
    }

    let value = publication_evidence().emit().unwrap();
    let recorded = PublicationEvidence::parse(&value)
        .unwrap()
        .payload_digest
        .to_string();
    ...
    let error = PublicationEvidence::parse(rebound.as_bytes()).unwrap_err();
...
fn publication_evidence_requires_immutable_deployment_resources() {
    ...
    let error = relative_record.emit().unwrap_err();
