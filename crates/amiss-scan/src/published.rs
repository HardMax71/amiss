...
use crate::discovery::component::{antora_anchor, antora_module, assembled};
...
pub(crate) fn anchors(
    ...
    match adapter {
        Adapter::AsciiDoc => Some(antora_anchor(snapshot, document, construct, path_part))
            .filter(|antora| !antora.is_empty() || !is_image)
            .unwrap_or_else(|| images_anchor(snapshot, document, path_part)),
