...
pub fn synthetic_candidate(
    ...
    let index_projection_digest = hb(
        INDEX_PROJECTION_SCHEMA,
        &serde_json::to_vec(&projection).map_err(|_defect| crate::Error::Internal)?,
    );
    ...
    let snapshot_digest = hb(
        SNAPSHOT_SCHEMA,
        &serde_json::to_vec(&input).map_err(|_defect| crate::Error::Internal)?,
    );
    Ok(IndexCandidate {
